...
const ActionTypeSchema = z.enum(['add_asset', 'ai_query']);

const UsageTypeSchema = z.enum(['ai', 'asset']);
...
function validateProfileUpdate(input) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return { valid: false, errors: error.errors.map(err => `${err.path.join('.')}: ${err.message}`) };
    }
    return { valid: false, errors: ['Validation error occurred'] };
  }
}
...
function validateSettings(input) {
  try {
    const validatedData = SettingsUpdateSchema.parse(input);
...
/**
 * Validate action type for limit checks
 */
function validateActionType(action) {
  try {
    ActionTypeSchema.parse(action);
    return true;
  } catch {
    return false;
  }
 ...
 * Validate usage type
 */
function validateUsageType(type) {
  try {
    UsageTypeSchema.parse(type);
    return true;
  } catch {
    return false;
  }
