...
// Lazy-initialized DynamoDB client for cold start optimization
 ...
 * @param {string} assetType - 'stock' or 'crypto'
 * @param {string} provider - Data provider name (default: alpaca)
 */
async function setQuote(symbol, quoteData, assetType = 'stock', provider = 'alpaca') {
  const cacheKey = `quote:${symbol.toUpperCase()}`;
...
/**
 * Get multiple quotes from cache (batch operation) - PARALLEL
...
async function getBatchQuotes(symbols, assetType = 'stock') {
  ...
  // Fetch all quotes in parallel for performance
 ...
 * @param {string} provider - Data provider name (default: alpaca)
 */
async function setBatchQuotes(quotes, assetType = 'stock', provider = 'alpaca') {
  const ttl = getQuoteTTL(assetType);
 ...
 * @param {string} provider - Data provider name (default: alpaca)
 */
async function setHistoricalPrices(symbol, prices, assetType = 'stock', provider = 'alpaca') {
 ...
 * @param {string} provider - Provider name (alpaca, gnews, etc.)
...
async function getQuotaUsage(provider) {
  ...
  try {
    ...
    if (result.Item) {
      return {
        ...
        dailyLimit: result.Item.dailyLimit || getDefaultLimit(provider),
  ...
  return { requestsUsed: 0, bandwidthUsed: 0, dailyLimit: getDefaultLimit(provider) };
}

/**
 * Get default daily limit for a provider
 * Alpaca Algo Trader Plus: 200 req/minute = ~288,000/day (practically unlimited)
 */
function getDefaultLimit(provider) {
  const limits = {
    'alpaca': 200000,      // Alpaca: effectively unlimited (200/min)
    'gnews': 100,          // GNews free tier
  ...
  try {
    await getDynamoClient().send(new UpdateCommand({
      ...
      ExpressionAttributeValues: {
        ...
        ':now': now,
        ':expires': expiresAt,
        ':limit': getDefaultLimit(provider),
      },
    }));
  } catch (e) {
...
async function getCacheStats() {
  // Get quota for all active providers
  // Alpaca is now the only market data provider
  const providers = ['alpaca', 'gnews'];
  const quotas = {};
